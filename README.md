...
Audio is generated by Dia. Pre-generated Dia clips for every move live under `audio/` with `audio_manifest.json`; run the Flask server (Dia backend) to stream or regenerate commentary. Newly generated clips land in `generated_audio/`.
